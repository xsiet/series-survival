...
import me.xsiet.survival.Whitelist
import me.xsiet.survival.survival
...
class EventListener(private val survival: Survival): Listener {
    @EventHandler
    ...
    fun onServerListPing(event: PaperServerListPingEvent) {
        ...
        event.maxPlayers = 0
        event.numPlayers = 0
        event.motd(text().color(TextColor.color(0xDD0000)).content("${ChatColor.BOLD}S U R V I V A L").build())
    }
    @EventHandler(ignoreCancelled = true)
